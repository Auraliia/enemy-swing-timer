...
local function __GetCategories(key, val)
    if key then EST_DEBUG_ENABLED_CATEGORIES[key] = val end
    return EST_DEBUG_ENABLED_CATEGORIES or {}
end

--- Get Enabled Category
--- @param key string The category key
local function __category(key)
    EST_DEBUG_ENABLED_CATEGORIES = EST_DEBUG_ENABLED_CATEGORIES or {}
    return EST_DEBUG_ENABLED_CATEGORIES[key]
end
...
local function __GetLogLevel(val)
    if val then EST_LOG_LEVEL = val end
    return EST_LOG_LEVEL or 0
end
...
--- @type Namespace
EST_NS = CreateNamespace(...)

--@do-not-package@
if kns.debug:IsDeveloper() then
    local p = EST_NS:CreateDefaultLogger('Ns')
    p:a(function() return '%s Namespace is: %s', kns.addon,
            c1(kns.sformat('EST_NS (%s)', type(EST_NS))) end)
end
--@end-do-not-package@
